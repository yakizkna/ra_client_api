...
- **杯赛（tour）**（`role:"cup"` 赛事管理 agent：`createCup`/`cupReport`/`endCup`/`reward` 管理
  全局八强淘汰杯赛；`create` 支持建空房/预占真实玩家 uid/`type:"tour"` 杯赛场次房；真人端在官网
  「杯」页报名后回调 `event:"tour_signup"`，由平台补位建场并按 8→4→2→1 编排，冠军真人胜者可获
  技能包奖品；赛事角色也可关闭超时比赛）
